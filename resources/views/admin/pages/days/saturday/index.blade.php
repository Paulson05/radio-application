        ...
        <div class="table-responsive">
            <table id="datatable" class="table table-striped table-bordered" cellspacing="0" width="100%">
                ...
                <tr>
                    ...
                    <td>{{$saturday->programme}}</td>
                    <td>{{$saturday->host}}</td>
                    <td><img src="{{$saturday->image}}" style="max-height: 90px; max-width: 50px;"></td>
                    <th>{{$saturday->time}}</th>
                    <td class="text-right">
                        <a href="{{route('saturday.edit', ['saturday'=>$saturday->id])}}" class=""><i class="fas fa-clock" style="color: red;"></i>edit</a>
                        <a href="{{route('saturday.show', ['saturday'=>$saturday->id])}}" class="btn btn-round btn-warning"><i class="fa fa-calendar-alt">show</i></a>
                        <form style="display: inline-block" method="post" action="{{route('saturday.destroy', ['saturday'=>$saturday->id])}}" >
                            @csrf
                            @method('DELETE')
